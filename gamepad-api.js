...
const EventEmitter = require('super-event-emitter/dist/super-event-emitter.js');
(function (window) {
    'use strict';
...
//console.log(pad.axes.toString());
        var left = {
            right: (Math.round(pad.axes[0]) === 1),
            left: (Math.round(pad.axes[0]) === -1),
            down: (Math.round(pad.axes[1]) === 1),
            up: (Math.round(pad.axes[1]) === -1)
        };
        var right = {
            right: (pad.axes[2] === 1),
            left: (pad.axes[2] === -1),
